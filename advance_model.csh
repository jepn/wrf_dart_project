...
#source /opt/modules/default/etc/modules.csh
#module swap xt-asyncpe xt-asyncpe/5.14
...
set  WRFICBCDIR = /work/jphacker/DART_WRF_INPUT
...
while($state_copy <= $num_states)
   ...
   if ( ( -d $temp_dir ) & ( $individual_members == "true" ) ) then
      ...
      ${COPY} ${CENTRALDIR}/wrflowinp_d0? .  ## Added by JAL (5 May 2014) for SST updates

   endif

   # link WRF-runtime files (required) and be.dat (if using WRF-Var)
#   ${LN} ${CENTRALDIR}/WRF_RUN/*       .
   ${COPY} ${CENTRALDIR}/WRF_RUN/*       .
...
### JPH sub wrfbdy_d01-->wrfinput_d01 for all IC files as LBCs
   ...
   while ( $keys[${ifile}] <= $wrfkey )
      echo looking for key $ifile $keys[${ifile}] $wrfkey
...
echo $END_SEC
      ...
      ### JPH need different format to name all-IC LBCs 
      ...
      echo copied ${CENTRALDIR}/WRF/wrfbdy_d01_${iday}_${isec}_mean
...
# The EOF on the line above MUST REMAIN in column 1.
         ...
         echo WARNING: Using perfect-model wrfinput files for fg

         ${LN} ${CENTRALDIR}/WRF_PERFECT/wrfinput_d01_${targdays}_${targsecs}_perfect ./fg

         ## JAL (19 Aug 2013):
         ## Command on Euler:
         ##mpirun ${CENTRALDIR}/WRF_RUN/da_wrfvar.exe >>&! out.wrfvar
         ## Command on Garnet:
         ##aprun -n 32 ${CENTRALDIR}/WRF_RUN/da_wrfvar.exe >>&! out.wrfvar
         ## Command on Yellowstone:
         mpirun.lsf ${CENTRALDIR}/WRF_RUN/da_wrfvar.exe >>&! out.wrfvar
...
# JPH No need to do this with using all-ICs LBCs
         echo $infl | ${CENTRALDIR}/update_wrf_bc >&! out.update_wrf_bc

      endif

      ${COPY} script.sed script.sed_pert_wrf_bc  ## JAL (14 Jan 2015)
...
# The EOF on the line above MUST REMAIN in column 1.
      ...
      ## JAL (15 Jan 2015): Ensure that wrflowinp files are present (they should be, but something was deleting them...)
      ##                    wrflowinp files are needed for SST updates and to prevent WRF crashing with num_land_cat mismatch errors
      ${COPY} ${CENTRALDIR}/wrflowinp_d0? .

      # run WRF here
      echo start, end times for WRF integration
      date
      ${ADV_MOD_COMMAND} >>&! rsl.out.integration
      echo advance status $status
      date
...
#      sleep 1
      ...
      while ( $dn <= $MY_NUM_DOMAINS )
         ...
         if ( ! -d ${CENTRALDIR}/WRFOUT/saveflds_${ensemble_member} ) /bin/mkdir -p ${CENTRALDIR}/WRFOUT/saveflds_${ensemble_member}
         
#         ls wrfout_d0${dn}_${END_STRING}
#         ls ${CENTRALDIR}/sfc_d0${dn}.cdl

#         set inFil = wrfout_d0${dn}_${END_STRING}
#         if ( -e inFil.nc ) /bin/rm inFil.nc
#         ln -sf $inFil inFil.nc 

         ncl inFilePath=\"wrfout_d0${dn}_${END_STRING}\" outFilePath=\"${CENTRALDIR}/WRFOUT/saveflds_${ensemble_member}/saveout_d0${dn}_${END_STRING}_${ensemble_member}\" cdlFilePath=\"${CENTRALDIR}/sfc_d0${dn}.cdl\" < ${CENTRALDIR}/strip_wrf_soil.ncl

#         ${MOVE} outFil.nc ${CENTRALDIR}/WRFOUT/saveflds_${ensemble_member}/saveout_d0${dn}_${END_STRING}_${ensemble_member}
#         ${REMOVE} inFil.nc
   ...
   if ( ! -e ${CENTRALDIR}/FILTER_ICS/${ensemble_member} ) /bin/mkdir -p ${CENTRALDIR}/FILTER_ICS/${ensemble_member}
   ${MOVE} dart_wrf_vector ${CENTRALDIR}/FILTER_ICS/${ensemble_member}/${output_file}
...
# $Revision: 4945 $
# $Date: 2011-06-02 15:29:30 -0700 (Thu, 02 Jun 2011) $
